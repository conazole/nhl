...
- jul 20 2026 (night) · the game-day layer (user: "do it all"). bet window on the slip: goalie-confirmation lamp (green all confirmed / amber n unconfirmed · check dfo) + live countdown to first puck (epoch-ms attribute · the iso string would be mangled by the year-strip shorthand). ⤢ focus mode strips the page to just the ticket. rank chips gained week movement (engine emits delta7 vs the ranking 7 days prior, same cached walk · tooltip says "↑4 wk"). yesterday's losses wear their bust-taxonomy chips with the bust_note in the tap tip. per-team 15-game tables fold closed inside game cards (goalie row + streak strip stay as the summary) and tapping a streak mark opens the fold and flashes the matching row. all tap-first; 47 tests green.
- jul 20 2026 (evening) · tap-first detail layer on the html board (user features; iphone-first · hover-only affordances are banned, tap is primary everywhere). rank chips reveal their u2.5 record ("u2.5 12/15 · ga 0.73/gp") via a shared tooltip (hover previews on desktop, tap pins on phone · capture-phase so a tap never toggles the accordion). line drift arrows on slip legs: when clv capture recorded a moved line, ↗/↘ shows market direction and the tap detail warns when the move crossed a line-factor boundary (thresholds from model_params · the gate-straddle trap surfaced on the ticket). tap targets grown to thumb size. tests cover the drift states, boundary flag, and clean degrade on pre-clv log entries.
- jul 20 2026 (later) · u2.5 team rank chips (user feature, revised same day). run_analysis.compute_team_rankings walks scoreboards backward through the shared scores cache (preseason excluded, olympic break skipped) and emits "team_rankings": each team's ROLLING LAST-15 u2.5 rate (user: teams change · current form beats a season-long running rank; 15 matches the report's window) with a least-1p-goals-allowed-per-game tiebreak, abbrev as the deterministic last resort; early-season teams rank on the games they have. build_html shows each team's rank in the game accordion titles ("buf #5 @ wsh #30"). display context only · never scored; a rankings failure never sinks the run and old engine json renders without chips. same-day display trims: slip title is just "u2.5", the confidence meter lives only on the ticket slip (bare numbers elsewhere, never "n/6" text), and the "day" tag never renders on the html (the start time carries it). tests cover the ordering rule, the chip fallback, and the tag filter.
