...
- jul 20 2026 (later) · season u2.5 team rank chips (user feature). run_analysis.compute_season_rankings walks the full season's scoreboards through the shared scores cache (preseason excluded, olympic break skipped) and emits "season_rankings": per-team u2.5 rate with a least-1p-goals-allowed-per-game tiebreak, abbrev as the deterministic last resort. build_html shows each team's rank in the game accordion titles ("buf #5 @ wsh #30"). display context only · never scored; a rankings failure never sinks the run and old engine json renders without chips. tests cover the ordering rule and the chip fallback.
