...
- analysis file order: masthead (title line, ━ rule, slate context line) → tonight at a glance → parlay / no-parlay / no-play → honorable mentions → avoid → yesterday + post-mortem → season · v4 → game details (collapsible <details> per game, sorted by confidence) → footer (model formula line + validated-through stamp from model_params.json).
