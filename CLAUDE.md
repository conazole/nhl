...
- REMOVED same day, do not re-add (user 2026-07-20): the slip's bet-window lamp line (goalie-confirmation status + first-puck countdown), the ⤢ focus button ("tacky"), and the bet badge on game rows ("hate it") · the slip is legs only, the ticket names the legs, and the pick panel inside a card marks a leg. goalie confirmation still lives in each game card's team rows and pick panel.
