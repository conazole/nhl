...
- self-fencing (user request, jul 3): maintenance.py runs in step 1 of every /nhl · 7-day counter auto-fires the weekly trio, season counter auto-fires the annual ritual; state in maintenance_state.json (never hand-edit). surface its drift alerts as a health line; incomplete ritual = do not bet. no cron by user preference · checkups ride the runs.
